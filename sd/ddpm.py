...
class DDPMSampler:
    ...
    def __init__(self,generator:torch.Generator,num_training_steps=1000, beta_start:float = 0.00085, beta_end = 0.0120):
    ...
    def step(self, timestep:int, latents:torch.Tensor, model_output:torch.Tensor):
        ...
        # find predicted original sample using formula (15) of DDPM paper
        pred_original_sample = (latents - beta_prod_t ** (0.5) * model_output)/alpha_prod_t ** (0.5)

        #compute coeff of pred_original_sample and current sample x_t
        pred_original_sample_coeff = (alpha_prod_t_prev ** 0.5 * current_beta_t)/beta_prod_t 
        current_sample_coeff = (current_alpha_t ** 0.5 * beta_prod_t_prev)/beta_prod_t 

        #find the previous predicted sample mean
    ...
    def add_noise(self, original_samples:torch.FloatTensor, timesteps:torch.IntTensor):
        ...
        #According to eqn (4) of DDPM paper (arxiv: 2006.11239)
        # Z ~ N(0,1) -> N(mean,var) = X ? , X = mean + stdev*Z
